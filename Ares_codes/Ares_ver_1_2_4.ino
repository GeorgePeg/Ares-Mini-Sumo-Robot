/*
  ...
  Description: Latest version of Ares for the robotic competition Robotic Arena with remote control using RC5 protocol. Here we also
  implement one more strategy. Before the start of the match, we push one of the board buttons to choose the strategy we want to have.
  The robot starts only when it receives the right signal (start signal) from the RC5 remote control.
...
void setup() {
  ...
  while (m == 0){
    ...
    */
    ...
    if (code == 0x1041 || code == 0x1042 || code == 0x1043) {
      started = true;
      debugMsg("Starting Routine...");
      }
      IrReceiver.resume();
    }
    delay(10);
